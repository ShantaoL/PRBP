...
__global__ void gpu_genbutterlies(FP *rand, FP *temp, int n) {
  ...
  for (int offset=0; offset <= 2*n; offset += 2*n) { 
	...
	temp[worker*2 + offset*2] = exp(rand[worker+offset]/10.);
	temp[worker*2+1 + offset*2] = exp(rand[worker+n/4+offset]/10.);
      }
       else {
	temp[worker*2+offset*2] = exp(rand[worker-n/4+offset]/10.0);
	temp[worker*2+1+offset*2] = -exp(rand[worker+offset]/10.0);
      }
      // Put the 0.5 scalar at the second matrix!
      if (worker < n/2) {
	temp[worker*2+2*n+offset*2] = exp(rand[worker+offset+n]/10.)/2.;
	temp[worker*2+2*n+1+offset*2] = exp(rand[worker+n*3/2+offset]/10.)/2.;
      }
      else {
	temp[worker*2+2*n+offset*2] = exp(rand[worker+n/2+offset]/10.)/2.;
	temp[worker*2+2*n+1+offset*2] = -exp(rand[worker+offset+n]/10.)/2.;
      } 
    } 
  }
}
...
int main(int argc, char *argv[]) {
  ...
  FP *a,*c;
  FP *dev_a, *dev_b, *dev_c, *dev_ub, *dev_r, 
  ...
  cublasDgemm(blasHandle, CUBLAS_OP_N, CUBLAS_OP_N, n, n, n,\
  ...
  cudaMalloc((void**)&dev_r, 4*n*(sizeof(FP)));
  ...
  // Step 2
  cusparseDcsrgemm(handle, notrans, notrans, n, n, n, descr, 2*n, dev_tmp, 
  ...
  cusparseDcsrgemm(handle, notrans, notrans, n, n, n, descr, 2*n, 
  ...
  cusparseDcsrmm(handle, trans, n, n, n, 4*n, &alpha, descr, dev_rresult,
  ...
  cusparseDcsr2dense(handle, n, n, descr, &dev_rresult[4*n], 
  ...
  */
  ...
  cudaMemcpy(dev_b, dev_a, n*n*sizeof(FP), cudaMemcpyDeviceToDevice);
  gpu_setdiag<<<RandGrid, RandBlock>>>(dev_b, n);
  
  cublasDtrsm(blasHandle, CUBLAS_SIDE_LEFT, CUBLAS_FILL_MODE_LOWER, \
      CUBLAS_OP_N, CUBLAS_DIAG_NON_UNIT, n, n, &alpha, dev_b, n, dev_c, n);

  cublasDtrsm(blasHandle, CUBLAS_SIDE_LEFT, CUBLAS_FILL_MODE_UPPER, \
      CUBLAS_OP_N, CUBLAS_DIAG_NON_UNIT, n, n, &alpha, dev_a, n, dev_c, n);
